...
func (s *CarStore) FindCars(
	...
	rawSql := `select cars.*, cars.id as car_id, cm.*
				...
				where ` + opt + ` customer_contracts.status = ? and cars.id not in (select car_id from customer_contracts where (customer_contracts.start_date >= ? and ? >= customer_contracts.start_date and (customer_contracts.status = 'ordered' or customer_contracts.status = 'renting' or customer_contracts.status = 'completed')) or partner_contracts.end_date < ?)`
	...
	rawSql = `select cars.*, cars.id as car_id, cm.*
				from cars inner join car_models cm on cars.car_model_id = cm.id inner join partner_contracts on cars.id = partner_contracts.car_id
				where ` + opt + ` customer_contracts.status = ? and cars.id not in (select car_id from customer_contracts where (? >= customer_contracts.start_date and customer_contracts.end_date >= ? and (customer_contracts.status = 'ordered' or customer_contracts.status = 'renting' or customer_contracts.status = 'completed')) or partner_contracts.end_date < ?)`
